...
function showCenterList(data) {
    ...
    data.forEach(function (item) { // item: 데이터 건 수
        ...
        fields.forEach(function (field) {
            // map => link생성.
            if (field == 'map') {
                let linkTag = document.createElement('a'); //link 태그
...
function showCenterListBySido(data) {
    ...
    centers.forEach(function (item) {
        //filteredCenter의 값중에서 item.sido 같은 값이 없으면 추가.
    ...
    function showSidoList() {
        // 서울특별시 버튼을 누르면 => 서울특별시 소속의 센터 정보를 출력
        ...
        // filterAry = [{},{},{},{}]
        pagingList();
        // showCenterList(filterAry); // 43개 호출. 10개씩 구분해서 넣기
    ...
    function searchArea() {
        ...
        let input = document.getElementById('searchCenter');
        let bt = document.querySelectorAll('#show>button');
        
        bt.forEach(function (item) {
            if (input == item.innerText) {
                item.innerText = bt.filter(function () {
                    item.click
                });
            }
        });
...
let url =
    `https://api.odcloud.kr/api/15077586/v1/centers?page=1&perPage=284&returnType=JSON&serviceKey=EfNQHxe7ABI4kcdRwMUNvFN0j%2Fbhj4rrfriyrV606EjC2Jp87%2BIN0g%2FU6Ewac0%2BE90pYpbBD5Y%2BSg1SvsfaWtA%3D%3D`;
...
xhtp.onreadystatechange = function () {
    if (xhtp.readyState == 4 && xhtp.status == 200) {
        ...
        showCenterListBySido(data); // 시/도별로 페이지 보여지기
